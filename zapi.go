...
func SetJob(h syscall.Handle, jobId uint32, level uint32, options *JOB_INFO_1, cmd uint32) (err error) {
	r1, _, e1 := syscall.Syscall6(procSetJobW.Addr(), 5, uintptr(h), uintptr(jobId), uintptr(level), uintptr(unsafe.Pointer(options)), uintptr(cmd), 0)
	if r1 == 0 {
		if e1 != 0 {
			err = error(e1)
		} else {
